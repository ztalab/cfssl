...
func loggerInit() {
	...
	if logConfig.Log.Level > 0 {
		logrus.SetLevel(logConfig.Log.Level)
	} else {
...
func LogDebugdw(logName string, df DynamicFields, msg string) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).Debug(fmt.Sprintf("%s: %s", getLogName(logName), msg))
}
...
func LogDebugdf(logName string, df DynamicFields, template string, args ...interface{}) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).Debugf(getLogName(logName)+": "+template, args...)
}
...
func LogInfodw(logName string, df DynamicFields, msg string) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).Info(fmt.Sprintf("%s: %s", getLogName(logName), msg))
}
...
func LogInfodf(logName string, df DynamicFields, template string, args ...interface{}) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).Infof(getLogName(logName)+": "+template, args...)
}
...
func LogWarndw(logName string, df DynamicFields, msg string) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).Warn(fmt.Sprintf("%s: %s", getLogName(logName), msg))
}
...
func LogWarndf(logName string, df DynamicFields, template string, args ...interface{}) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).Warnf(getLogName(logName)+": "+template, args...)
}
...
func LogErrord(logName string, df DynamicFields, msg string) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).WithField("logger", traceFunc()).Error(fmt.Sprintf("%s: %s", getLogName(logName), msg))
}
...
func LogErrordw(logName string, df DynamicFields, msg string, err error) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).WithField("logger", traceFunc()).Error(fmt.Sprintf("%s: %s, %s", getLogName(logName), msg, err.Error()))
}
...
func LogErrordf(logName string, df DynamicFields, template string, args ...interface{}) {
	logrus.WithFields(Fields).WithFields(structToMap(df)).WithField("logger", traceFunc()).Errorf(getLogName(logName)+": "+template, args...)
}
