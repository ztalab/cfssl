...
func (c *ciCore) combineFields(fields []zapcore.Field) map[string]interface{} {
	// Copy our map.
	m := make(map[string]interface{}, len(c.fields)+len(c.fields))
...
func (c *ciCore) Write(e zapcore.Entry, fields []zapcore.Field) error {
	fields = append(fields,
		zap.String(fieldTime, e.Time.Format(defaultTimestampFormat)),
		zap.String(fieldThreadId, strconv.FormatUint(getGID(), 10)),
		zap.String(fieldLogger, e.Caller.TrimmedPath()+" "+e.Caller.Function),
		zap.String(fieldCustomLog2, e.Stack),
	)
	...
	for field, value := range combinedFields {
		if _, ok := allowedFields[field]; !ok {
			...
			delete(combinedFields, field)
		}
	}

	if len(additionFields) > 0 {
		additionValues, err := jsoniter.Marshal(additionFields)
		if err != nil {
			fmt.Printf("[ERR]: addition values marshal err: %v", err)
		} else {
			message.WriteString(" ")
			message.Write(additionValues)
		}
	}
